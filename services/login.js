...
function handle_request(msg, callback){
	var res = {};
	console.log("In RMQS Login Q:In handle request:"+ msg);
	var email=msg.email;
	var password=msg.password;
	// var email=msg.req.param("email");
	// var password=msg.req.param("password");
	if(msg.login_message==="signin"){
	console.log("In RMQS:Inside signIn function");
	console.log("Recieved Login request from user user id::" + email);
	...
	else if(msg.login_message==="signup")
	{
		...
		console.log("In RMQS:Inside signup function");
		var data=msg.data;
		console.log("In RMQS"+data+"From Client");
		var dbConn = mysql.getDBConn();
		if(dbConn === "empty")
		{
	...
	{
		console.log("In RMQS:In handle request Signout:"+ msg);
		var userid=msg.userid;
		var data = 
		{
			lastLogin : new Date()
		};
		console.log(userid+ "Test" + data);
		var dbConn = mysql.getDBConn();
		if(dbConn === "empty")
		{
			...
			var query = dbConn.query("Update users set ? where user_id=? ", [ data,userid ], function(err, rows) {
				...
				else 
				{
					if (rows.affectedRows > 0)
					{
						...
						//req.session.destroy(function(err){ "Commented By me"
							// if(err)
							// {
								// console.log(err);
							// }
...
function getSummary(user_id, req, res, callback)
{
	...
	} else 
	{
		var query = dbConn.query("select summary from users where user_id = ? ", [ user_id ], function(err, rows) {
			...
			getExperience(localSession.user_id, req, res, callback);
			mysql.returnDBconn(dbConn);
		});
	}
}
...
function getExperience(user_id, req, res, callback)
{
	var dbConn = mysql.getDBConn();
	if(dbConn === "empty")
	{
	...
	} else 
	{
		var query = dbConn.query("select * from experience where user_id = ? ", [ user_id ], function(err, rows) {
			...
			getEducation(localSession.user_id, req, res, callback);
			getConnections(localSession.user_id, req, res, callback);
			mysql.returnDBconn(dbConn);
		});
	}
}
...
function getEducation(user_id, req, res, callback)
{
	var dbConn = mysql.getDBConn();
	if(dbConn === "empty")
	{
	...
	} else 
	{
		var query = dbConn.query("select * from education where user_id = ? ", [ user_id ], function(err, rows) {
			...
			getSkills(dbConn, localSession.user_id, req, res, callback);
			mysql.returnDBconn(dbConn);
		});
	}
}
...
function getConnections(user_id, req, res, callback)
{
	var dbConn = mysql.getDBConn();
	if(dbConn === "empty")
	{
...
function getSkills(dbConn, user_id, req, res, callback)
{
	var query = dbConn.query("select * from skills where user_id = ?", [ user_id ], function(err, rows) {
		...
		//checkInvitations(dbConn, user_id, req, res); "Commented by Me"
